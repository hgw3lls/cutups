...
The Python GUI is split into **Render**, **SuperCollider**, and **Parameters** tabs. The **Render** tab can start a render, show the live command, follow progress, and precompute an analysis cache with `Analyze` so later renders can reuse source descriptors in readonly mode. Cache analysis writes its own progress JSONL and updates the same progress bar/ETA area used by renders. The **SuperCollider** tab sends OSC to `127.0.0.1:57120` by default. Use `Load live`, `Load chunks`, `Refresh chunks`, `Live start`, `Live stop`, `Scene on/off`, `Stab`, `Phrase`, `Concrete`, `Send age`, and `Stop all` from the same window that starts the render. It can also control SuperCollider-native cutups with `SC input on/off`, `SC cuts on/off`, `SC stab`, `SC glitch`, source buttons for `Material`, `Live`, or `Blend`, SC performance presets, SC cue buttons that apply a preset and start a matching layer stack, `Bed on/off/rev` for long tape-bed loops, held bed slots `A`, `B`, and `C`, and feedback buttons for overdubbing SC cuts/bed loops back into the rolling live buffer.

The tape deck also includes performance ideas retained from older TRANSMISSIONS SuperCollider sketches: a `CNCR` concrete-cut trigger, tape `AGE` in the master strip, and OSC triggers such as `/cutups/stab`, `/cutups/phrase`, `/cutups/concrete`, `/cutups/scene`, `/cutups/stop`, `/cutups/loadLive`, `/cutups/loadChunks`, `/cutups/chunks/refresh`, `/cutups/live/start`, `/cutups/live/stop`, `/cutups/live/amp`, `/cutups/scLive/input/start`, `/cutups/scLive/input/stop`, `/cutups/scLive/cuts/start`, `/cutups/scLive/cuts/stop`, `/cutups/scLive/stab`, `/cutups/scLive/glitch`, `/cutups/scLive/monitor`, `/cutups/scLive/overdub`, `/cutups/scLive/decay`, `/cutups/scLive/source`, `/cutups/scLive/preset`, `/cutups/scLive/cue`, `/cutups/scLive/bed/start`, `/cutups/scLive/bed/stop`, `/cutups/scLive/bed/reverse`, `/cutups/scLive/bed/slot`, `/cutups/scLive/bed/slot/clear`, and `/cutups/master/age`. The dedicated `LIVE TRACK` layer continuously reloads/restarts the growing `*_live_track.wav` with a short fade instead of interrupting the fragment buffers. The `SC PRE` selector applies safe parameter snapshots: `balanced`, `signal-breach`, `ghost-transmission`, `beat-stutter`, and `spoken-word`. These presets change source mode, levels, tape-strip tone, bed timing, overdub, and decay without starting or stopping layers. The cue buttons use the same names but also start useful layer combinations for breach, ghost, beat, and spoken-word performance states. The `SC SRC` selector lets `SC CUTS` and `BED LOOP` use loaded material, the rolling live-input buffer, or a blend. `SC INPUT` can stay off for file/folder-only cutups, or be toggled on to add live sound into the blend. `ODUB` feeds SC-native cut and bed output back into the live-input buffer; `DECAY` controls how long older buffer material remains. `BED LOOP` adds long drifting tape loops with reverse, dropout, wow/flutter, and wait/duration controls. Held bed slots `A`, `B`, and `C` capture persistent loop instances from the current `SC SRC`; `CLR` frees the held slots.
...
python3 PY/cutup.py --mode audio --preset beat-cutup --input ./loops --analysis-cache ./loops/audio_analysis_cache.json --analysis-cache-readonly --output out/beat_demo
```
